...
if(process.argv[1]&&fileURLToPath(import.meta.url)===resolve(process.argv[1])) {
  ...
  const summaryTimer=setInterval(summaryTick,60000);
  summaryTimer.unref();
  void summaryTick();

  server.listen(Number(process.env.PORT||8080),'0.0.0.0',()=>console.log('Formtech service listening'));
  for(const signal of ['SIGTERM','SIGINT'])process.on(signal,()=>{clearInterval(timer);clearInterval(summaryTimer);server.close(()=>process.exit(0));});
}
