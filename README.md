...
## Five-hour callback reminders
`FOLLOWUP_EMAIL_ENABLED=true` enables one email per phone enquiry still marked New with Callback needed after five elapsed hours from creation (overnight/weekends count). Auckland goes to martin@formtech.co.nz; Christchurch and unspecified locations go to jason@formtech.co.nz. Existing overdue calls are included when enabled. Uses the same RESEND_API_KEY and CALL_SUMMARY_FROM as daily summaries, independently of CALL_SUMMARY_ENABLED.

Completed callbacks, local status changes, archived or deleted records suppress the reminder. A linked HubSpot ticket must have a successful progress check in the last three minutes and still be in the configured initial New stage; progress or unavailable status suppresses sending. Assignment alone is not a status change. Edits to notes or due dates alone do not reset the five-hour clock. The worker checks every 30 seconds; larger backlogs can take multiple cycles.

Delivery uses a persistent queue with frozen payloads and idempotency keys, retry backoff, and a 23-hour retry cutoff to avoid duplicate messages. A progress change cancels pending retries. After acceptance, an activity note records the recipient; acceptance is not confirmed inbox delivery. No repeat reminder is created when a record is reopened. Disable with FOLLOWUP_EMAIL_ENABLED=false.
